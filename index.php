<?php 
    ...
    if (isset($_GET['category_id'])) {
        ...
        $statement = $db->prepare("SELECT blogs.id, blogs.image, blogs.title, blogs.content, blogs.created_at, users.name FROM blogs INNER JOIN users ON blogs.user_id = users.id WHERE blogs.category_id = $categoryId ORDER BY blogs.id DESC");
    } else {
        $statement = $db->prepare("SELECT blogs.id, blogs.image, blogs.title, blogs.content, blogs.created_at, users.name FROM blogs INNER JOIN users ON blogs.user_id = users.id ORDER BY blogs.id DESC");
    }
    $statement->execute();
